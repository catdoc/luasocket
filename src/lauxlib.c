...
static lua_ext_file_loader g_ext_file_loader = NULL;

void lua_ext_set_file_loader(lua_ext_file_loader loader)
{
    g_ext_file_loader = loader;
}
...
LUALIB_API int luaL_loadstringex(lua_State *L, const char* s, unsigned int size) {
    const char* ofLuaMark = "OFLUA_1";
    const unsigned int ofLuaMarkSize = strlen(ofLuaMark);

    char file_header[8];
    memcpy(file_header, s, 7);
    file_header[7] = 0;
    if (strcmp(ofLuaMark, file_header) != 0) {
        return luaL_loadbufferx(L, s, size, NULL, NULL);
    }

    unsigned int contentSize = size - ofLuaMarkSize;
    char* encryptedData = (char*)malloc(contentSize + 1);
    memcpy(encryptedData, s + ofLuaMarkSize, contentSize);

    size_t outLen = 0;
    char* outData = (char*)xxtea_decrypt(encryptedData, contentSize, "OF_LUA_201801", &outLen);
    int ret = luaL_loadbufferx(L, outData, outLen, NULL, NULL);

    // test string data.
    /*char* testStr = malloc(outLen + 1);
    memcpy(testStr, outData, outLen);
    testStr[outLen] = '\0';
    printf(testStr);
    free(testStr);*/
...
LUALIB_API int luaL_loadfilex (lua_State *L, const char *filename,
                                             const char *mode) {
  const char* pExt = filename + strlen(filename) - 5;
  if (0 == strcmp(pExt, "oflua")) {
    FILE* file = fopen(filename, "rb");
    if (!file)
    {
        if (g_ext_file_loader)
        {
            int cnt = 0;
            char* encryptedData = g_ext_file_loader(filename, &cnt);
            if (encryptedData == NULL)
            {
                luaL_error(L, "failed to load file: %s", filename);
                return -1;
            }
            int ret = luaL_loadstringex(L, encryptedData, cnt);// || lua_pcall(L, 0, LUA_MULTRET, 0);
            free(encryptedData);
            /*if (ret)
            {
    ...
    char file_header[ 8 ];
    fread(file_header, 1, 7, file);
    file_header[ 7 ] = 0;
    if (strcmp("OFLUA_1", file_header) != 0) {
      fclose(file);
      return -1;
    }

    char* encryptedData = (char*) malloc(filesize + 1 );
    size_t readCount = fread(encryptedData, 1, filesize, file);
    fclose(file);
    size_t outLen = 0;
    char* outData = (char*) xxtea_decrypt(encryptedData, readCount, "OF_LUA_201801", &outLen);
    int ret = luaL_loadbufferx(L, outData, outLen, NULL, NULL);
    free(encryptedData);
    free(outData);
    return ret;
  ...
  if (filename == NULL) {
    ...
    if (lf.f == NULL)
    {
        ...
        {
            int cnt = 0;
            char* fdata = g_ext_file_loader(filename, &cnt);
            if (fdata == NULL)
            {
                luaL_error(L, "failed to load file: %s", filename);
                return -1;
            }
            int ret = luaL_loadstringex(L, fdata, cnt);// || lua_pcall(L, 0, LUA_MULTRET, 0);
            free(fdata);
            return ret;
        }
